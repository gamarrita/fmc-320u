...
uint32_t WRPEndOffset, FunctionalState WRPLock);
...
static void FLASH_OB_RDPKeyConfig(uint32_t RDPKeyType, uint32_t RDPKey1,
uint32_t RDPKey2);
/**
 * @}
 */
...
HAL_StatusTypeDef HAL_FLASHEx_OBProgram(FLASH_OBProgramInitTypeDef *pOBInit)
{
    ...
    if (status == HAL_OK)
    {
        ...
        if ((pOBInit->OptionType & OPTIONBYTE_WRP) != 0U)
        {
            ...
            pOBInit->WRPEndOffset, pOBInit->WRPLock);
        }
        ...
        if ((pOBInit->OptionType & OPTIONBYTE_RDPKEY) != 0U)
        {
            ...
            FLASH_OB_RDPKeyConfig(pOBInit->RDPKeyType, pOBInit->RDPKey1,
            pOBInit->RDPKey2);
        }
...
void HAL_FLASHEx_GetOperation(FLASH_OperationTypeDef *pFlashOperation)
{
    ...
    & (FLASH_OPSR_SYSF_OP | FLASH_OPSR_BK_OP);
...
static void FLASH_OB_UserConfig(uint32_t UserType, uint32_t UserConfig)
{
    ...
    if ((UserType & OB_USER_IO_VDD_HSLV) != 0U)
    {
        ...
        assert_param(
        IS_OB_USER_IO_VDD_HSLV(UserConfig & FLASH_OPTR_IO_VDD_HSLV));
    ...
    if ((UserType & OB_USER_IO_VDDIO2_HSLV) != 0U)
    {
        ...
        assert_param(
        IS_OB_USER_IO_VDDIO2_HSLV(UserConfig & FLASH_OPTR_IO_VDDIO2_HSLV));
...
uint32_t *WRPEndOffset, FunctionalState *WRPLock)
{
    ...
    if (WRPArea == OB_WRPAREA_BANK1_AREAA)
    {
        ...
        *WRPLock =
        (READ_BIT(FLASH->WRP1AR, FLASH_WRP1AR_UNLOCK) != 0U) ? DISABLE : ENABLE;
    }
    else if (WRPArea == OB_WRPAREA_BANK1_AREAB)
    {
        ...
        *WRPLock =
        (READ_BIT(FLASH->WRP1BR, FLASH_WRP1BR_UNLOCK) != 0U) ? DISABLE : ENABLE;
    }
    else if (WRPArea == OB_WRPAREA_BANK2_AREAA)
    {
        ...
        *WRPLock =
        (READ_BIT(FLASH->WRP2AR, FLASH_WRP2AR_UNLOCK) != 0U) ? DISABLE : ENABLE;
    }
    else if (WRPArea == OB_WRPAREA_BANK2_AREAB)
    {
        ...
        *WRPLock =
        (READ_BIT(FLASH->WRP2BR, FLASH_WRP2BR_UNLOCK) != 0U) ? DISABLE : ENABLE;
    }
    else
    {
        /* Empty statement (to be compliant MISRA 15.7) */
